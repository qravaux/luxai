...
class Agent():
    def __init__(self, player: str, env_cfg) -> None:
        ...
        self.policy.load_state_dict(torch.load("../../jax_based_agent/policy/separated_unit_networks_rng/policy_step_117.pth", weights_only=True))
    ...
    def act(self, step: int, obs, remainingOverageTime: int = 60):
        ...
        if self.player == 'player_0' :
            points = jnp.clip(obs['team_points'][0] - self.previous_obs['team_points'][0],min=0)
        else :
            points = jnp.clip(obs['team_points'][1] - self.previous_obs['team_points'][1],min=0)
        self.previous_obs = obs
