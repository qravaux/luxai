...
class Luxai_Worker(mp.Process) :
    ...
    def __init__(self, worker_id, shared_queue, policy_0, policy_1, victory_bonus,gamma,gae_lambda) :
    ...
    def run(self) :
        ...
        with torch.no_grad():
            ...
            while True:
                ...
                values = torch.zeros(2,self.len_episode,1,dtype=torch.double)
                ...
                for step in range(self.len_episode):
                    ...
                    values[0,step] = value_0
                    values[1,step] = value_1
                ...
                advantages = torch.zeros(2,self.len_episode,1)
                
                last_gae_lam_0 = 0
                last_gae_lam_1 = 0

                for step in reversed(range(self.len_episode)):

                    if step == self.len_episode - 1 :
                        next_values_0 = values[0,-1]
                        next_values_1 = values[1,-1]
                    else:
                        next_values_0 = values[0,step + 1]
                        next_values_1 = values[1,step + 1]

                    delta_0 = rewards_0[step] + self.gamma * next_values_0 - values[0,step]
                    delta_1 = rewards_1[step] + self.gamma * next_values_1 - values[1,step]

                    last_gae_lam_0 = delta_0 + self.gamma * self.gae_lambda * last_gae_lam_0
                    last_gae_lam_1 = delta_1 + self.gamma * self.gae_lambda * last_gae_lam_1

                    advantages[0,step] = last_gae_lam_0
                    advantages[1,step] = last_gae_lam_1

                returns = advantages + values
                #returns = (cumulated_rewards[:,-1].view(2,1) - cumulated_rewards).view(2,self.len_episode,1)
                ...
                self.shared_queue.put((states,actions,advantages,returns,cumulated_rewards[:,-1]))
...
if __name__ == "__main__":
    ...
    for i in range(num_workers) :
        worker = Luxai_Worker(i, shared_queue, model_0, model_1, victory_bonus,gamma,gae_lambda)
    ...
    for epoch in range(n_epochs):
        ...
        for _ in range(n_episode):
            ...
            states,actions,advantages,returns,cumulated_rewards = shared_queue.get()

            experiences_0.append((states[0],actions[0],advantages[0],returns[0],cumulated_rewards[0]))
            experiences_1.append((states[1],actions[1],advantages[1],returns[1],cumulated_rewards[1]))

        # Process the collected experiences
        states_0,actions_0,advantages_0,returns_0,cumulated_rewards_0 = zip(*experiences_0)
        states_1,actions_1,advantages_1,returns_1,cumulated_rewards_1 = zip(*experiences_1)
        ...
        for i in range(n_episode) :
            writer.add_scalar("Reward agent 0", cumulated_rewards_0[i].item(), step_cpt + i * len_episode)
            writer.add_scalar("Reward agent 1", cumulated_rewards_1[i].item(), step_cpt + i * len_episode)
